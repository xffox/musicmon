...
shouldProduce playerStates expSongs = do
    (Maybe.catMaybes $ map snd $ reverse $ foldl step [] playerStates)
        `shouldBe` expSongs
  where
    step prevs ps = (MusicState.stepMusicState (state prevs) ps) : prevs
...
spec = describe "music state" $ do
    it "yields a song when enough is played" $ do
        ...
        [ (Model.StatePlaying song, startTime)
            , (Model.StateStopped, ts)
            ]
            `shouldProduce` [Model.PlayedSong ts song]
    it "doesn't yield a song when not enough is played" $ do
        let song = prepareSong "Painkiller" 15
        [ (Model.StatePlaying song, startTime)
            , (Model.StateStopped, timeAddSeconds startTime 6)
            ]
            `shouldProduce` []
    it "yields a paused and stopped song" $ do
        ...
        [ (Model.StatePlaying song, startTime)
            ...
            , (Model.StateStopped, timeAddSeconds startTime 13)
            ]
            `shouldProduce` [Model.PlayedSong (timeAddSeconds startTime 13) song]
    it "yields multiple songs" $ do
        ...
        [ (Model.StatePlaying song1, startTime)
            ...
            , (Model.StateStopped, timeAddSeconds startTime 17)
            ]
            `shouldProduce` [ Model.PlayedSong (timeAddSeconds startTime 8) song1
    ...
    it "yields paused and continued song" $ do
        ...
        [ (Model.StatePlaying song, startTime)
            ...
            , (Model.StateStopped, timeAddSeconds startTime 9)
            ]
            `shouldProduce` [Model.PlayedSong (timeAddSeconds startTime 9) song]
    it "doesn't yield a paused song when not enough is played" $ do
        let song = prepareSong "Painkiller" 15
        [ (Model.StatePlaying song, startTime)
            , (Model.StatePaused song, timeAddSeconds startTime 3)
            , (Model.StatePlaying song, timeAddSeconds startTime 15)
            , (Model.StateStopped, timeAddSeconds startTime 18)
            ]
            `shouldProduce` []
